...
func DisplayInvestingFAQ() {
	...
	green.Println("A: A SAFE with valuation cap protects investors:")
	fmt.Println("   • Cap is set at 65% of current valuation (minimum $500k)")
	fmt.Println("   • If company raises above cap, you convert at cap price")
	fmt.Println("   • Example: Invest $100k with $1M cap")
	fmt.Println("   • Company raises at $2M → you convert at $1M cap (better terms!)")
	fmt.Println("   • Protects early investors from overvaluation")
	...
	fmt.Println("\nQ: What is Employee Option Pool Dilution?")
	green.Println("A: When companies raise new rounds, they typically set aside")
	fmt.Println("   15-20% of post-money valuation for employee stock options:")
	fmt.Println("   • This dilutes ALL existing shareholders (founders + investors)")
	fmt.Println("   • Standard practice to attract and retain talent")
	fmt.Println("   • Happens automatically in each funding round")
	fmt.Println("   • Example: $10M Series A → $2M option pool → everyone diluted")
	...
	fmt.Println("\nQ: What is Carry Interest?")
	green.Println("A: VC's share of profits (typically 20%):")
	fmt.Println("   • You keep 80% of profits, LPs get 20%")
	fmt.Println("   • Only applies to profits above hurdle rate (usually 8%)")
	fmt.Println("   • Example: $1M fund → $5M exit → $4M profit")
	fmt.Println("   • You get: $3.2M (80%), LPs get: $0.8M (20%)")
	fmt.Println("   • Displayed in portfolio dashboard with projections")
	
	fmt.Println("\nQ: What are LP Commitments?")
	green.Println("A: Limited Partners commit capital to your fund:")
	fmt.Println("   • LPs commit 2x your initial fund size")
	fmt.Println("   • Capital calls happen quarterly (25% per call)")
	fmt.Println("   • Example: $1M fund → $2M committed → $500k per quarter")
	fmt.Println("   • Tracked in portfolio dashboard")
	fmt.Println("   • Realistic fund management like real VCs")
	...
	fmt.Println("\nQ: How do I predict ROI?")
	green.Println("A: ROI Predictor analyzes:")
	fmt.Println("   • Current growth trajectory from funding rounds")
	fmt.Println("   • Risk factors (lower risk = higher confidence)")
	fmt.Println("   • Market conditions and sector trends")
	fmt.Println("   • Historical performance patterns")
	fmt.Println("   • Provides: Current ROI, Projected ROI, Best/Worst case")
	fmt.Println("   • Confidence level: 0-100% based on data quality")
	fmt.Println("   • More rounds = more data = higher confidence")
	
	yellow.Println("\n🎯 NEW FEATURES")
	fmt.Println()
	fmt.Println("Q: What is Employee Option Pool Dilution?")
	green.Println("A: NEW FEATURE - Realistic dilution mechanics:")
	fmt.Println("   • Companies set aside 15-20% of post-money for employees")
	fmt.Println("   • Automatically dilutes all shareholders in each round")
	fmt.Println("   • Standard practice in real VC deals")
	fmt.Println("   • Makes dilution calculations more realistic")
	
	fmt.Println("\nQ: How does ROI Predictor work?")
	green.Println("A: NEW FEATURE - Projected ROI calculator:")
	fmt.Println("   • Analyzes growth trends from funding rounds")
	fmt.Println("   • Adjusts for risk factors and market conditions")
	fmt.Println("   • Projects best case, worst case, and expected ROI")
	fmt.Println("   • Provides confidence level based on data quality")
	fmt.Println("   • Helps make informed follow-on investment decisions")
	
	fmt.Println("\nQ: Can I end my affiliate program?")
	green.Println("A: NEW FEATURE - End Affiliate Program (Founder Mode):")
	fmt.Println("   • Shut down affiliate program anytime")
	fmt.Println("   • Option to transition customers to direct sales")
	fmt.Println("   • Or let affiliate customers churn")
	fmt.Println("   • Useful when program isn't performing or too expensive")
	
	fmt.Println("\nQ: What is a Customer Referral Program?")
	green.Println("A: NEW FEATURE - Paid customer referrals (Founder Mode):")
	fmt.Println("   • Customers refer new customers for rewards")
	fmt.Println("   • Requires 10+ customers to launch")
	fmt.Println("   • Setup cost: $10-30k, monthly platform fee: $2-5k")
	fmt.Println("   • Reward per referral: $500-$2000 (you set)")
	fmt.Println("   • 60-80% conversion rate from referrals to customers")
	fmt.Println("   • Scales with customer base (more customers = more referrals)")
	fmt.Println("   • Lower cost than affiliate program, but less scalable")
...
func DisplayHelpGuide() {
	...
	fmt.Printf("%s Employee Option Pool: 15-20%% dilution per round (NEW!)\n", ascii.Star)
	fmt.Printf("%s Post-Money Valuation: Pre-money + investment\n", ascii.Chart)
	fmt.Printf("%s Valuation Caps: SAFE investments with cap protection (NEW!)\n", ascii.Shield)
	fmt.Printf("%s Carry Interest: 20%% of profits above hurdle rate\n", ascii.Medal)
	fmt.Printf("%s LP Commitments: Quarterly capital calls from LPs (NEW!)\n", ascii.Calendar)
	fmt.Printf("%s ROI Predictor: Projected ROI calculator (NEW!)\n", ascii.Chart)
